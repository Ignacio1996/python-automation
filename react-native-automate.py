...
react_navigation = 'yarn add react-navigation  @react-navigation/native react-native-reanimated react-native-gesture-handler react-native-screens react-native-safe-area-context @react-native-community/masked-view @react-navigation/stack @react-navigation/drawer && '
ui_kitten = 'yarn add @ui-kitten/components@next @eva-design/eva@next react-native-svg @ui-kitten/eva-icons@next && '
pod_install = 'pwd && cd ios && pod install && cd .. && '
firebase = 'yarn add firebase && mkdir firebase && touch firebase/index.js && '
